...
import { FaMapMarkerAlt, FaUser, FaUtensils, FaCalendarAlt, FaBoxOpen, FaSearch, FaFilter } from 'react-icons/fa';
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredDonations.length === 0 ? (
            <div className="col-span-full py-12 text-center">
              ...
              <p className="text-gray-500 text-lg">No food donations match your search.</p>
              <button 
            ...
            filteredDonations.map((donation) => {
              ...
              return (
                <div
                  key={donation._id}
                  className={`bg-white rounded-xl border ${donation.isReserved ? 'border-gray-200' : 'border-green-200'} shadow hover:shadow-md transition-all p-5 flex flex-col justify-between relative overflow-hidden`}
                >
                  {/* Status badge */}
                  ...
                  <div className="space-y-4 mt-4">
                    <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                      <FaUtensils className="text-green-600" /> {donation.foodType}
                    </h3>
                    ...
                    <p className="text-gray-700 flex items-center gap-2">
                      <FaCalendarAlt className={`${expiryColorClass} min-w-4`} /> 
                      <span className="text-gray-500">Expiry:</span> 
                      <span className={`font-medium ${expiryColorClass}`}>
                        {new Date(donation.expiryDate).toLocaleDateString()}
                        {daysUntilExpiry <= 2 && daysUntilExpiry >= 0 && " (Soon)"}
                        {daysUntilExpiry < 0 && " (Expired)"}
                        {daysUntilExpiry > 2 && ` (${daysUntilExpiry} days left)`}
                      </span>
                    </p>
                  </div>
